...
<div class="glass-card rounded-2xl p-8 max-w-2xl mx-auto">
    <div class="text-center mb-10">
        <div class="w-16 h-16 primary-gradient rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-emerald-100">
    ...
    <form action="{{ route('tickets.search') }}" method="GET" class="space-y-6">
        <div>
            ...
            <input type="text" name="ticket_number" required
                class="w-full px-6 py-4 text-center text-2xl font-bold tracking-widest rounded-2xl border border-slate-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-all placeholder:text-slate-300 placeholder:font-normal placeholder:tracking-normal"
                placeholder="TK-YYYYMMDD-XXXX">
        </div>

        <div class="pt-4">
            <button type="submit"
                class="w-full primary-gradient text-white font-bold py-4 rounded-xl shadow-lg shadow-emerald-200 hover:shadow-emerald-300 transform transition-all active:scale-95">
                ค้นหาสถานะ
            </button>
        </div>
    </form>
    ...
    <div class="mt-10 pt-8 border-t border-slate-100 text-center">
        ...
        <a href="{{ route('tickets.create') }}" class="inline-block mt-4 text-emerald-600 font-bold hover:underline">กลับไปหน้าแจ้งซ่อม</a>
    </div>
</div>
@endsection
