...
<div class="max-w-5xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="glass-card rounded-3xl overflow-hidden">
                ...
                <div class="p-8 space-y-8">
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        ...
                        <div>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">ผู้รับผิดชอบ</p>
                    ...
                    <div>
                        ...
                        <a href="{{ Storage::url($ticket->attachment_path) }}" target="_blank" 
                           class="inline-flex items-center px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all">
        ...
        <!-- Action Sidebar -->
        <div class="space-y-6">
            <div class="glass-card rounded-3xl p-8 border-2 border-indigo-50 shadow-xl shadow-indigo-100/50">
                ...
                <div class="mb-8 p-4 bg-indigo-50 rounded-2xl border border-indigo-100 text-center">
                    <p class="text-sm text-indigo-700 font-medium mb-4">ใบงานนี้ยังไม่มีผู้รับผิดชอบ</p>
                    <form action="{{ route('admin.tickets.accept', $ticket->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="w-full bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition-all">
                            กดเพื่อรับใบงาน
                        </button>
                    </form>
                </div>
                ...
                <form action="{{ route('admin.tickets.updateStatus', $ticket->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="status" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all bg-white font-medium">
                            <option value="pending" {{ $ticket->status == 'pending' ? 'selected' : '' }}>รอรับเรื่อง</option>
                            ...
                            <option value="more_info" {{ $ticket->status == 'more_info' ? 'selected' : '' }}>ต้องการข้อมูลเพิ่ม</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">หมายเหตุจากเจ้าหน้าที่</label>
                        <textarea name="admin_note" rows="5" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder:text-slate-300" 
                                  placeholder="ระบุความคืบหน้าหรือข้อความถึงผู้แจ้ง...">{{ $ticket->admin_note }}</textarea>
                    </div>

                    <button type="submit" class="w-full primary-gradient text-white font-bold py-4 rounded-xl shadow-lg shadow-indigo-100 hover:shadow-indigo-200 transform transition-all active:scale-95">
                        บันทึกการอัปเดต
                    </button>
                </form>
            </div>
            ...
            <div class="p-6 bg-slate-900 rounded-3xl text-white">
                <h4 class="font-bold text-indigo-400 mb-2">💡 ข้อแนะนำ</h4>
