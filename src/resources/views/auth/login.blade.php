...
<div class="max-w-md mx-auto py-10">
    <div class="glass-card rounded-3xl overflow-hidden shadow-2xl shadow-indigo-100/50">
        <div class="primary-gradient p-8 text-center text-white">
            ...
            <p class="text-indigo-100 text-sm mt-2 opacity-80">สำหรับเจ้าหน้าที่ไอทีเท่านั้น</p>
        </div>
        ...
        <div class="p-8">
            ...
            <form action="{{ route('login.post') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="email" name="email" value="{{ old('email') }}" required autofocus
                            class="w-full pl-12 pr-4 py-3.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                ...
                <button type="submit" 
                    class="w-full primary-gradient text-white font-bold py-4 rounded-xl shadow-lg shadow-indigo-200 hover:shadow-indigo-300 transform transition-all active:scale-[0.98]">
                    เข้าสู่ระบบ
                </button>
            </form>
        </div>
    ...
    <div class="mt-8 text-center">
        <a href="{{ route('tickets.create') }}" class="text-slate-500 hover:text-indigo-600 text-sm font-medium transition-colors flex items-center justify-center">
